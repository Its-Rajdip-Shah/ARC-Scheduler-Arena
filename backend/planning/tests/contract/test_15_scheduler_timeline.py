...
@covers("SCH-008")
def test_SCH008_workload_pressure_is_effort_based_not_task_count_based(
    user, make_item
):
    """Scheduler must have duration/effort semantics available to placement."""
    fields = {f.name for f in PlanningItem._meta.get_fields()}
    assert "duration_category" in fields, (
        "SCH-008: workload cannot be effort-based without canonical duration."
    )

    tiny = make_item(user, "Tiny", duration_category="UNDER_20_MINUTES")
    huge = make_item(user, "Huge", duration_category="OVER_16_HOURS")
