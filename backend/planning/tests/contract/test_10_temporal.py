...
@covers('TMP-001', 'TMP-002', 'TMP-003')
...
@covers("TMP-005")
def test_TMP005_distant_deadline_does_not_automatically_outrank_healthy_priority(
    user, make_item, today
):
    high = make_item(
        user,
        "Higher priority flexible",
        due_date=today + timedelta(days=30),
    )
    lower = make_item(
        user,
        "Lower priority with distant deadline",
        due_date=today + timedelta(days=20),
    )

    priority.reconcile(user)
    before = list(priority.ordered(user).values_list("pk", flat=True))
    assert before.index(high.pk) < before.index(lower.pk)

    _reschedule(user, today)

    after = list(priority.ordered(user).values_list("pk", flat=True))
    assert after.index(high.pk) < after.index(lower.pk), (
        "TMP-005: merely having a somewhat nearer but still healthy deadline "
        "must not silently rewrite canonical priority."
    )


@covers("TMP-006")
def test_TMP006_deadline_constrained_work_is_kept_inside_legal_window(
    user, make_item, today
):
    release = today
