...
def test_each_contract_area_is_claimed_by_its_designated_test_file():
    ...
    for filename, areas in TEST_FILE_AREAS.items():
        ...
        if not path.exists():
            # The existence test gives the cleaner primary failure.
            continue

        claimed = _literal_contract_ids_from_file(path)
        required = set().union(*(ids_for_area(area) for area in areas))
        missing = sorted(required - claimed)
        if missing:
            problems[filename] = {"missing_contract_ids": missing}

    assert not problems, (
        "Some frozen requirements have no test in their designated module: "
        f"{problems}"
    )
